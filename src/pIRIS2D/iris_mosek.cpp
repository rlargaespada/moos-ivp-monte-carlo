...
void extract_solution(double* xx, double* barx, int n,
                      std::vector<int> ndx_d, IRISEllipse *ellipse)
{
  int dimension{2};  // ellipse is in 2D
  int bar_ndx = 0;
  for (int j=0; j < 2*n; j++) {
    for (int i=j; i < 2*n; i++) {
      if ((j < dimension) && (i < dimension)) {
        ellipse->setCEntry(i, j, barx[bar_ndx]);
        ellipse->setCEntry(j, i, barx[bar_ndx]);  // since barx is just the lower triangle
      }
      bar_ndx++;
    }
  }
  ...
  for (int i=0; i < dimension; i++) {
    ellipse->setDEntry(i, xx[ndx_d[i]]);
  }
}


double inner_ellipsoid(const IRISPolygon &polygon, IRISEllipse *ellipse, MSKenv_t *existing_env)
{
  MSKenv_t *env;
  if (existing_env) {
    env = existing_env;
  ...
  const int m = polygon.getNumConstraints();
  const int n = 2;  // problem is in 2D
  ...
  for (int i=0; i < m; i++) {
    ...
    for (int j=0; j < n; j++) {
      ...
      for (int k=0; k < n; k++) {
        ...
        bara_v[abar_ndx + k] = polygon.getA()(i, k);
      }
      abar_ndx += n;
    ...
    for (int j=0; j < num_d; j++) {
      ...
      vali_A_row[j] = polygon.getA()(i, j);
    }
    subi_A_row[num_d] = ndx_g[i];
    vali_A_row[num_d] = 1;
    check_res(MSK_putarow(task, con_ndx, num_d + 1, subi_A_row.data(), vali_A_row.data()));
    check_res(MSK_putconbound(
      task, con_ndx, MSK_BK_FX, polygon.getB()(i, 0), polygon.getB()(i, 0)));
    con_ndx++;
  }
  ...
  for (int j=0; j < num_s; j++) {
    ...
    check_res(MSK_appendcone(
      task, MSK_CT_RQUAD, 0.0, 3, csub));  // 3rd argument (0.0) is reserved for future use by Mosek
    lhs_idx += 2;
  }
  ...
  for (int i=0; i < m; i++) {
    ...
    // 3rd argument (0.0) is reserved for future use by Mosek
    check_res(MSK_appendcone(
      task, MSK_CT_QUAD, 0.0, n + 1, csub_f_row.data()));
  }
  ...
  check_res(MSK_putbarablocktriplet(
    task, nabar, bara_i.data(), bara_j.data(), bara_k.data(), bara_l.data(), bara_v.data()));
  ...
  switch (solsta) {
    ...
    // case MSK_SOL_STA_NEAR_OPTIMAL:
      ...
      extract_solution(xx, barx, n, ndx_d, ellipse);
    ...
    case MSK_SOL_STA_UNKNOWN:
      std::cout << "Inner ellipsoid: The status of the "
                   "solution could not be determined." << std::endl;
      std::cout << "A: " << std::endl << polygon.getA() << std::endl;
      std::cout << "b: " << polygon.getB().transpose() << std::endl;
      throw(InnerEllipsoidInfeasibleError());
    default:
...
Eigen::Vector2d closest_point_in_convex_hull(const Eigen::Matrix2Xd &Points, MSKenv_t *existing_env)
{
  ...
  switch (solsta) {
    ...
    // case MSK_SOL_STA_NEAR_PRIM_INFEAS_CER:
      ...
      // std::cout << "Closest point in convex hull: "
      //              "The status of the solution could not be determined." << std::endl;
      // if (!existing_env) {
      //   MSK_deleteenv(env);
      //   free(env);
      // }
